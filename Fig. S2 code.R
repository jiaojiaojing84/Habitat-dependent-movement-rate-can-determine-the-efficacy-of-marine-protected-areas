...
for(i in 1:length(r))
{
  for(j in 1:length(m))
  {
  	for(z in 1:S)
  	{
  	parameters <- c(T=1,L=9,r=1,e=0.01,r1=1,e1=0.01,mu=0.5,D1=m[j]*r[i],D2=m[j],F=0.25) 
  	out= ode(y = inits, times = times, func = JAP08, parms = parameters)
  	eqn1[i,j,z]<-out[Timesteps+1,z+1]
  	
  	parameters <- c(T=5,L=5,r=1,e=0.01,r1=1,e1=0.01,mu=0.5,D1=m[j]*r[i],D2=m[j],F=0.25) 
  	out= ode(y = inits, times = times, func = JAP08, parms = parameters)
  	eqn5[i,j,z]<-out[Timesteps+1,z+1]
  	
  	parameters <- c(T=9,L=1,r=1,e=0.01,r1=1,e1=0.01,mu=0.5,D1=m[j]*r[i],D2=m[j],F=0.25) 
  	out= ode(y = inits, times = times, func = JAP08, parms = parameters)
...
L1high<-mean(eqn1[1,1,10])/mean(eqn1[1,1,1:9])
L2high<-mean(eqn1[2,1,10])/mean(eqn1[2,1,1:9])
L3high<-mean(eqn1[3,1,10])/mean(eqn1[3,1,1:9])

####local effect calculation
L1media<-mean(eqn5[1,1,6:10])/mean(eqn5[1,1,1:5])
L2media<-mean(eqn5[2,1,6:10])/mean(eqn5[2,1,1:5])
L3media<-mean(eqn5[3,1,6:10])/mean(eqn5[3,1,1:5])

##local effect calculation
L1small<-mean(eqn9[1,1,2:10])/mean(eqn9[1,1,1])
L2small<-mean(eqn9[2,1,2:10])/mean(eqn9[2,1,1])
L3small<-mean(eqn9[3,1,2:10])/mean(eqn9[3,1,1])


tiff("Close_system_Fig.S2.tiff", width=5,height=5, units='in',res=600)
par(mfrow=c(3,3))
par(mar=c(2,2,1.5,2))
...
points(seq(1,10,1),eqn9[1,1,]/den_bef,lwd=1,type="p",col="black")
points(1,yval6[1],col="#CF3721",pch=16)
abline(v=1.5,lwd=2,col="black",lty=5)
text(4,1,paste("LE=",round(mean(eqn9[1,1,2:10])/mean(eqn9[1,1,1]),digits=2)))

yval7<-eqn9[2,1,]/den_bef
plot(seq(1,10,1),eqn9[2,1,]/den_bef,lwd=2,type="p",lty=2,ylim=c(min(bind),max(bind)),col="#31A9B8",pch=16)
points(seq(1,10,1),eqn9[2,1,]/den_bef,lwd=1,type="p",col="black")
points(1,yval7[1],col="#CF3721",pch=16)
abline(v=1.5,lwd=2,col="black",lty=5)
text(4,1,paste("LE=",round(mean(eqn9[2,1,2:10])/mean(eqn9[2,1,1]),digits=2)))

yval8<-eqn9[3,1,]/den_bef
plot(seq(1,10,1),eqn9[3,1,]/den_bef,lwd=2,type="p",lty=3,ylim=c(min(bind),max(bind)),col="#31A9B8",pch=16)
points(seq(1,10,1),eqn9[3,1,]/den_bef,lwd=1,type="p",col="black")
points(1,yval8[1],col="#CF3721",pch=16)
abline(v=1.5,lwd=2,col="black",lty=5)
text(4,1,paste("LE=",round(mean(eqn9[3,1,2:10])/mean(eqn9[3,1,1]),digits=2)))
...
text(3.1,1.5,paste("LE=",round(mean(eqn5[1,1,6:10])/mean(eqn5[1,1,1:5]),digits=2)))

yval4<-eqn5[2,1,]/den_bef
plot(seq(1,10,1),eqn5[2,1,]/den_bef,lwd=2,type="p",lty=2,ylim=c(min(bind),max(bind)),pch=16,col="#31A9B8")
points(seq(1,10,1),eqn5[2,1,]/den_bef,lwd=1,type="p",col="black")
points(1:5,yval4[1:5],pch=16,lwd=2,col="#CF3721")
abline(v=5.5,lwd=2,col="black",lty=5)
text(3.1,1.5,paste("LE=",round(mean(eqn5[2,1,6:10])/mean(eqn5[2,1,1:5]),digits=2)))

yval5<-eqn5[3,1,]/den_bef
plot(seq(1,10,1),eqn5[3,1,]/den_bef,lwd=2,type="p",lty=3,ylim=c(min(bind),max(bind)),col="#31A9B8",pch=16)
points(seq(1,10,1),eqn5[3,1,]/den_bef,lwd=1,type="p",col="black")
points(1:5,yval5[1:5],lwd=2,pch=16,col="#CF3721")
abline(v=5.5,lwd=2,col="black",lty=5)
text(3.1,1.5,paste("LE=",round(mean(eqn5[3,1,6:10])/mean(eqn5[3,1,1:5]),digits=2)))
...
text(3.5,1.4,paste("LE=", round(mean(eqn1[1,1,10])/mean(eqn1[1,1,1:9]),digits=2)))
...
abline(v=9.5,lwd=2,col="black",lty=5)
text(3.5,1.4,paste("LE=",round(mean(eqn1[2,1,10])/mean(eqn1[2,1,1:9]),digits=2)))
...
abline(v=9.5,lwd=2,col="black",lty=5)
text(3.5,1.4,paste("LE=",round(mean(eqn1[3,1,10])/mean(eqn1[3,1,1:9]),digits=2)))
